...
\t'cmake'
\t'ros-build-tools'
\t${ros_makedepends[@]}%(other_build_dependencies)s
)
...
"""
    ...
    def generate(self, python_version, exclude_dependencies=[], rosdep_urls=[],
        ...
        pkgbuild = self.BUILD_TEMPLATE % {
          ...
          'ros_build_dependencies': ''.join(['\n\t' + x for x in ros_build_dep]),
          'ros_run_dependencies': ''.join(['\n\t' + x for x in ros_run_dep]),
          'other_build_dependencies': ''.join(['\n\t' + x for x in other_build_dep]),
          'other_run_dependencies': ''.join(['\n\t' + x for x in other_run_dep]),
...
def main():
    ...
    parser.add_option(
        ...
        metavar='rosdep_urls',
        default=[
            default_rosdep_url % 'base', default_rosdep_url % 'python',
            default_rosdep_url % 'ruby'
        ],
        help='The URLs of the rosdep mapping files.')
    parser.add_option(
        '--exclude-dependencies',
        metavar='exclude_dependencies',
        default='',
